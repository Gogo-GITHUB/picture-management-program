...
public class ThreadImages implements Runnable {
    ...
    int k;
	 ...
	 public ThreadImages(ArrayList<File> ImageFiles, int k, int i, ArrayList<JLabel> SmallLabels, ArrayList<JTextField> SmallTextFields, ArrayList<JPanel> SmallPanels, JPanel ImagePanel) {
        this.k = k;
    ...
    public synchronized void run() {
        ...
        for (int j = k; j < i&&j<ImageFiles.size(); j++) {
            ...
            if (h1 < 77 && w1 < 100) {
                ...
                ic2.setImageObserver(SmallLabels.get(j));//设置图像的图像观察者
                SmallTextFields.get(j).setText(ImageFiles.get(j).getName());//设置名字

            } else {
                if (h1 * 180 > w1 * 142) {
                    Image im = ic1.getImage().getScaledInstance((int) (w1 / (h1 / 81)), 81, Image.SCALE_DEFAULT);//改变大小，创建此图像的缩放版本
                    ImageIcon ic2 = new ImageIcon(im);//从新得到一个固定图片
                    ...
                    Image im = ic1.getImage().getScaledInstance(105, (int) (h1 / (w1 / 105)), Image.SCALE_DEFAULT);//改变大小，创建此图像的缩放版本
            ...
            if (ImageFiles.size()>20) {
                SmallPanels.get(j).setBounds(j % 5 * 131, 1 + (j / 5 * 125), 120, 110);//设置边界
            } else {
            ...
            SmallLabels.get(j).setOpaque(true);//组件绘制其边界内的每个像素
            SmallLabels.get(j).setBackground(new java.awt.Color(48,48,48));
        }
